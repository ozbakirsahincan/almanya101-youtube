  ...
  <div className="glass-card p-6 hover:border-white/12 transition-all duration-300">
    <div className="flex items-start justify-between mb-4">
      <div className="flex-1">
        <Badge variant="accent" className={EVENT_TYPE_COLORS[event.type]}>{event.type}</Badge>
        <h3 className="font-display font-bold text-xl text-white mt-3">{event.title}</h3>
    ...
    <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/6">
      <button className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-all">
        <i className="fas fa-info-circle" /><span>Details</span>
      </button>
      <button className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-white/8 text-white/60 hover:text-white hover:border-white/12 transition-all">
        <i className="fas fa-calendar-plus" /><span>{event.attendees?.length > 0 ? 'Leave' : 'Join'}</span>
      </button>
    </div>
  </div>
);
...
const EventsPage = () => {
  ...
  const { data: eventsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode()) return getMockEventsData();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="bento-input w-auto px-4 appearance-none cursor-pointer">
          <option value="">All Events</option>
          {Object.keys(EVENT_TYPE_COLORS).filter((t) => t !== 'other').map((t) => (
            <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}s</option>
          ))}
        </select>
        <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 px-4 h-10 rounded-lg bg-indigo-500 text-white font-medium hover:bg-indigo-600 hover:shadow-glow-hover transition-all">
          <i className="fas fa-plus" /><span>Create Event</span>
        </button>
      </div>
      {isLoading ? (
